...
const char * const BITCOIN_CONF_FILENAME = "spice.conf";
const char * const BITCOIN_PID_FILENAME = "spiced.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "spice" is a composite category enabling all Spice-related debug output
            if(ptrCategory->count(string("spice"))) {
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty spice.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override spice.conf
