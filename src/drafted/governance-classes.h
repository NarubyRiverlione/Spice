 ...
 *
 *   // network
 *   CSpiceNetwork lvl, network-type, network-status, network-error, milestone-status*
...
private:
    ...
    // Current OBJECT STATUS (see http://govman.spice.org/index.php/Documentation_:_Status_Field)
    int nStatusID;
...
// // root node
class CSpiceNetwork : public CGovernanceObject
{
private:
    std::string strName;
    std::string strURL;
...
// // can be under: SpiceNetwork
// //   -- signature requirements : Key1(User)
// class CSpiceNetworkVariable : public CGovernanceObject
// {
// private:
